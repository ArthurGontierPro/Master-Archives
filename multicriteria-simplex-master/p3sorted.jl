...
function isinsort(L,B)
	return B in L
end

function p3sorted(C,A,b,B1)
	# construction de la liste de bases et la liste de solutions
	LB = [B1]
	Lx = []

	nbv = size(A,2) # nb de variables
	nbc = size(A,1) # nb de contraintes
	nbo = size(C,1) # nb d' objectifs

	cb = 1 # indice de la base courante
	# tant qu'il nous reste des bases dans la liste L à explorer
	while cb <= length(LB)
		B = LB[cb] # base courante
		N = [n for n in 1:nbv if !(n in B)] # N : variables hors base
		...
		# décomposition LU
		L,U,p = lu(AB)# p est le vecteur des permutations que fais julia pour la decomp LU
